...
`cloudbuild.yaml` は **`docker/Dockerfile.fullstack`** をビルドします。1 コンテナ内で **Nginx（`$PORT` で待受）→ 静的フロント**、**`/fastapi/` は同一コンテナ内の Uvicorn（127.0.0.1:8000）**へプロキシします。

Cloud Run の URL（`https://....run.app`）で次を確認できます。
...
| `docker/Dockerfile.cloudrun` | Cloud Run 向け API のみのイメージ |
| `docker/Dockerfile.fullstack` | Cloud Run 向け Nginx + フロント + FastAPI |
| `docker/nginx.conf` | Compose 用: 静的配信と `/fastapi/` プロキシ |
| `docker/nginx.fullstack.conf` | フルスタック用 Nginx（`__PORT__` を entrypoint で置換） |
| `docker/entrypoint-api.sh` | Compose 用: DB 待機・スキーマ・Uvicorn |
| `docker/entrypoint-fullstack.sh` | フルスタック: Uvicorn + Nginx 起動 |
